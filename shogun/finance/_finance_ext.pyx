...
cdef class InnerPosition:
    ...
    def __repr__(self):
        return (
            '%s(instrument=%r, amount=%r, cost_basis=%r,'
...
cpdef update_position_last_sale_prices(positions, get_price, dt):
    ...
    get_price : callable[Instrument, float]
        The function to retrieve the price for the instrument.
    dt : pd.Timestamp
    ...
    for outer_position in itervalues(positions):
        ...
        last_sale_price = get_price(inner_position.instrument)
...
cdef class PositionStats:
    ...
    position_exposure_series : pd.Series[float64]
        The exposure of each position in the same order as
        ``position_tracker.positions``. The index is the numeric exchange_symbol of each
        instrument.
    Notes
    -----
...
cpdef calculate_position_tracker_stats(positions, PositionStats stats):
    ...
    for outer_position in itervalues(positions):
        ...
        with cython.boundscheck(False), cython.wraparound(False):
            index[ix] = position.instrument.to_esid()
