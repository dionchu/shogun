...
class RollFinder(with_metaclass(ABCMeta, object)):
    ...
    def get_rolls(self, root_symbol, start, end, offset, active):
        ...
        if back is not None:
            ...
            first = front
        first_contract = oc.exchange_symbol_to_contract[first]
        rolls = [((first_contract >> offset).contract.exchange_symbol, None)]
        ...
        while session > start and curr is not None:
            front = curr.contract.exchange_symbol
            back = rolls[0][0]
            prev_c = curr.prev
            while session > start:
                prev = session - freq
                ...
                if back != self._active_contract(oc, front, back, prev):
                    ...
                    rolls.insert(0, ((curr >> offset).contract.exchange_symbol, session))
                    break
                session = prev
            curr = curr.prev
            if curr is not None:
...
class CalendarRollFinder(RollFinder):
    ...
    def __init__(self, trading_calendar, instrument_finder):
        self.trading_calendar = trading_calendar
        self.instrument_finder = instrument_finder

    def _active_contract(self, oc, front, back, dt):
        contract = oc.exchange_symbol_to_contract[front].contract
...
class VolumeRollFinder(RollFinder):
    ...
    def __init__(self, trading_calendar, instrument_finder, session_reader):
        self.trading_calendar = trading_calendar
        self.instrument_finder = instrument_finder
    ...
    def _active_contract(self, oc, front, back, dt):
        ...
        """
        front_contract = oc.exchange_symbol_to_contract[front].contract
        back_contract = oc.exchange_symbol_to_contract[back].contract
