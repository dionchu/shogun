...
from functools import partial #in continuous_futures.py

from numpy import array, empty, iinfo  #in continuous_futures.py
from numpy cimport long_t, int64_t  #NOT in continuous_futures.py
from pandas import Timestamp  #in continuous_futures.py
from trading_calendars import get_calendar  #in continuous_futures.py
import warnings  #in continuous_futures.py


def delivery_predicate(codes, contract): #OK
...
march_cycle_delivery_predicate = partial(delivery_predicate,
                                         set(['H', 'M', 'U', 'Z'])) #NOT IN CONTINUOUS_FUTURES.PY

CHAIN_PREDICATES = { #NOT IN CONTINUOUS_FUTURES.PY
...
ADJUSTMENT_STYLES = {'add', 'mul', None} #OK
...
cdef class ContinuousFuture:
    ...
    _kwargnames = frozenset({
        'exchange_symbol', #CHANGED FROM SID TO EXCHANGE_SYMBOL TO MATCH CONTINUOUS_FUTURES.PY
        'root_symbol',
        'offset',
        'start_date',
        'end_date',
    ...
    def __init__(self, #OK
                 exchange_symbol, # changed from long_t sid to exchange_symbol
                 root_symbol, # removed object XXX
                 offset, # removed object XXX
                 roll_style, # removed object XXX
                 start_date, # removed object XXX
                 end_date, # removed object XXX
                 exchange_info, # removed object XXX
                 adjustment=None, # removed object XXX
                 active=True): # ADDED active = True

        self.exchange_symbol = exchange_symbol #CHANGED FROM SID TO EXCHANGE_SYMBOL TO MATCH CONTINUOUS_FUTURES.PY
        self.exchange_symbol_hash = hash(exchange_symbol) #CHANGED FROM SID TO EXCHANGE_SYMBOL TO MATCH CONTINUOUS_FUTURES.PY
